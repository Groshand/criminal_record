                ...
                @foreach($offense as $ofns)
                        @foreach($users as $user)
                            @if($ofns->userId==$user->id)
                                <button data-toggle="modal" data-target="#a{{$ofns->id}}" class="col-md-12 btn border-secondary  @if($ofns->accept==0)btn-success @elseif($ofns->accept==1)btn-danger @elseif($ofns->accept==2) btn-primary @endif" style="border-style:solid;
    border-width:thick;">
                                    <div class="row ">
                                        <div class="col-md-3">
